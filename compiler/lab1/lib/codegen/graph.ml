...
let print (g : t) = 
  let print_aux ~key:v ~data:s = 
    let s_str = Vertex.Set.fold s ~init:"" ~f:(fun acc -> fun u -> acc ^ (Vertex._to_string u) ^ ", ")in
...
let weights_max_vertex weights = 
  ...
  let res_opt = List.max_elt w_list ~compare:(fun (_, w1) -> fun (_, w2) -> Int.compare w1 w2) in
    match res_opt with None -> raise (Failure "max should not be called on empty") | Some (vmax, _) -> vmax
;;
...
;;
let live_in_out_aux (l : AS.instr) acc = 
  match acc with 
    [] -> raise (Failure "live_in_out_aux should not take in empty initial liveinfo")
  | liveinfo_line :: _ -> 
...
let live_in_out (prog : AS.instr list) = 
  List.fold_right prog ~f:live_in_out_aux ~init:[liveinfo_line_init]
;;
