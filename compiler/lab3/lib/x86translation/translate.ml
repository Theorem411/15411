...
let translate_cmp get_final = function
  | AS.Cmp { rhs; lhs; size } ->
    ...
    | X86.Imm _, X86.Imm _ ->
      [ X86.BinCommand { op = Mov; src = lf; dest = X86.get_free size; size }
      ; X86.BinCommand { op = Mov; src = rf; dest = X86.get_memfree size; size }
      ; X86.Cmp { lhs = X86.get_free size; rhs = X86.get_memfree size; size }
      ]
    | X86.Imm _, _ ->
      [ X86.BinCommand { op = Mov; src = lf; dest = X86.get_free size; size }
      ; X86.Cmp { lhs = X86.get_free size; rhs = rf; size }
...
let translate_line
  ...
  | LLVM_Call _ | LLVM_Cmp _ | LLVM_IF _ | LLVM_Ret _ | LLVM_Jmp _ | LLVM_Set _ ->
    prev_lines
  | LLVM_ArrayIdxCheck _ | LLVM_NullCheck _ | LLVM_MovFrom _ | LLVM_MovTo _ -> prev_lines
;;
