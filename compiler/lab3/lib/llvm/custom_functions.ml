...
let set_unsafe b = unsafe_ref := b
let get_unsafe () = !unsafe_ref
let mac = false
...
let get_efkt_name_ops = function
  ...
  | "alloc_array" -> CustomAssembly.alloc_array_fname ~unsafe:(get_unsafe ())
  | "alloc" -> CustomAssembly.alloc_fname ~unsafe:(get_unsafe ())
...
let format_check_null () =
  if get_unsafe ()
  then "define void @" ^ get_efkt_name_ops "check_null" ^ "(ptr %ptr) {\n ret void\n}\n"
  else
    "define void @"
...
let format_alloc () =
  ...
  then
    ...
    ^ "(i32 noundef %x) local_unnamed_addr #0 {\n\
       %x_i64 = sext i32 %x to i64\n\
       %res = tail call ptr @calloc(i64 noundef 1, i64 noundef %x_i64) #4\n\
       ret ptr %res\n\
       } "
  else
    "; Function Attrs: nounwind ssp uwtable(sync)\ndefine noalias ptr @"
...
let format_alloc_array () =
  ...
  then
    ...
    ^ "(i32 noundef %0, i32 noundef %1) local_unnamed_addr #3 {\n\
      \      %3 = mul nsw i32 %1, %0\n\
      \      %4 = sext i32 %3 to i64\n\
      \      %5 = add nsw i64 %4, 8\n\
      \      %6 = tail call ptr @calloc(i64 noundef 1, i64 noundef %5) #5\n\
    ...
    ^ "(i32 noundef %cstm_0, i32 noundef %cstm_1) local_unnamed_addr #0 {\n\
       ...
       c0_alloc_array13:                                               ; preds = \
       %cstm_11, %cstm_8, %cstm_6\n\
