...
let detect_cycle (graph : SS.t SM.t) =
  let rec dfs node visited_states =
    ...
    | None ->
      ...
      visited_states
        := SM.mapi
             ~f:(fun ~key:n ~data:state -> if Symbol.equal n node then Visited else state)
             !visited_states
  in
...
let rec static_semantic_exp (mexp : A.mexp) (exp_ctx : exp_ctx) : exp_res =
  ...
  | A.CmpBinop { op; lhs; rhs } ->
    ...
    | 8, 8 ->
      ...
      (match tl, tr with
      | T.Array _, T.Array _ -> ()
      | T.Array _, _ | _, T.Array _ -> failwith "one side is array other is sth else"
      | _ -> ());
