...
(*_ cfg funciton *)
let to_cfg (fspace : fspace) : Cfg.cfg_input =
  let { block_info : block list; _ } = fspace in
  let entry =
    List.filter_map fspace.block_info ~f:(fun blc ->
      let l = blc.label in
      match l with
      | Label.FunName _ -> Some l
      | _ -> None)
  in
  let entry =
    if List.length entry <> 1 then failwith "impossible!" else List.hd_exn entry
  in
  let nodes = List.map block_info ~f:(fun { label; _ } -> label) in
  let edges : Cfg.edge_t list =
    List.fold block_info ~init:[] ~f:(fun acc blc ->
      match blc.jump with
      | JRet -> acc
      | JCon { jt; jf } ->
        ({ pred = blc.label; chld = Label.bt jt } : Cfg.edge_t)
        :: { pred = blc.label; chld = Label.bt jf }
        :: acc
      | JUncon l -> { pred = blc.label; chld = Label.bt l } :: acc)
  in
  let block_cfg : Cfg.cfg_input = { entry; nodes; edges } in
  block_cfg
;;

let tuse_remove_ln (tuse : IS.t TH.t) (t : Temp.t) (ln : int) : unit = 
  let update_f = function 
  | None -> failwith "impossible! temp is already gone when tuse_remove is called"
  | Some iset -> IS.remove iset ln 
  in
  TH.update tuse t ~f:update_f
;;

let tuse_add_ln (tuse : IS.t TH.t) (t : Temp.t) (ln : int) : unit = 
  let update_f = function 
  | None -> failwith "impossible! temp is not in tuse when tuse_add is called"
  | Some iset -> IS.add iset ln
  in 
  TH.update tuse t ~f:update_f
;;
