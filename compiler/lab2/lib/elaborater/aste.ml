...
module Print = struct
  ...
  let rec pp_stm ?(n = 0) stm = 
    let f ~n = 
      function
      | Declare {var; typ; body} -> sprintf "%s %s;\n%s" (Typ._tostring typ) (Symbol.name var) (pp_program ~n:(n + 1) body)
      | Assign {var; exp} -> sprintf "%s = %s;" (Symbol.name var) (pp_mexp exp)
      | If {cond; lb; rb} -> sprintf "if (%s) {\n%s}{\n  %s}" (pp_mexp cond) (pp_program ~n:(n + 1) lb) (pp_program ~n:(n + 1) rb)
